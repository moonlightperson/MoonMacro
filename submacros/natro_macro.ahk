...
nm_UpdateButton(*)
{
	...
	if (CopyPaths = 1)
	{
		try
		{
			...
			wr.Open("GET", "https://api.github.com/repos/moonlightperson/MoonMacro/tags?per_page=100", 1)
			...
			wr := ComObject("WinHttp.WinHttpRequest.5.1")
			wr.Open("GET", "https://api.github.com/repos/moonlightperson/MoonMacro/compare/" base "..." latest_release["tag_name"] , 1)
			wr.SetRequestHeader("accept", "application/vnd.github+json")
			wr.Send()
			wr.WaitForResponse()
...
for k,v in ["PMondoGuid","PMondoGuidComplete","PFieldBoosted","PFieldGuidExtend","PFieldGuidExtendMins","PFieldBoostExtend","PPopStarExtend","PScorchStarExtend"]
	%v%:=0
...
; check for update
try AsyncHttpRequest("GET", "https://api.github.com/repos/moonlightperson/MoonMacro/releases/latest", nm_AutoUpdateHandler, Map("accept", "application/vnd.github+json"))
; open Timers
if (TimersOpen = 1)
...
try AsyncHttpRequest("GET", "https://raw.githubusercontent.com/moonlightperson/.github/main/data/beesmas.txt", nm_BeesmasHandler, Map("accept", "application/vnd.github.v3.raw"))
;Blender
MainGui.SetFont("w700")
...
GitHubRepoLink(*){
	Run "https://github.com/moonlightperson/MoonMacro"
}
GitHubReleaseLink(*){
	Run "https://github.com/moonlightperson/MoonMacro/releases"
}
nm_RunDiscord(path){
...
nm_ReportBugButton(*){
	Run "https://github.com/moonlightperson/MoonMacro/issues/new?assignees=&labels=bug%2Cneeds+triage&projects=&template=bug.yml"
}
nm_MakeSuggestionButton(*){
	Run "https://github.com/moonlightperson/MoonMacro/issues/new?assignees=&labels=suggestion%2Cneeds+triage&projects=&template=suggestion.yml"
}
blc_mutations(*) {
	global
	local script, exec
...
nm_GoGather(){
	global youDied, VBState
		...
		, MicroConverterKey
		, WhirligigKey, PFieldBoosted, GlitterKey, GatherFieldBoosted, GatherFieldBoostedStart, LastGlitter, PMondoGuidComplete, LastGuid, PMondoGuid, PFieldGuidExtend, PFieldGuidExtendMins, PFieldBoostExtend, PPopStarExtend, PScorchStarExtend, HasPopStar, HasScorchStar, PopStarActive, ScorchStarActive, FieldGuidDetected, ConvertGatherFlag
		, LastWhirligig
	...
	while(((nowUnix()-gatherStart)<(FieldUntilMins*60)) || (PFieldBoosted && (nowUnix()-GatherFieldBoostedStart)<840) || (PFieldBoostExtend && (nowUnix()-GatherFieldBoostedStart)<1800 && (nowUnix()-LastGlitter)<900) || (PFieldGuidExtend && FieldGuidDetected && (nowUnix()-gatherStart)<(FieldUntilMins*60+PFieldGuidExtend*60) && (nowUnix()-GatherFieldBoostedStart)>900 && (nowUnix()-LastGlitter)>900) || (PPopStarExtend && HasPopStar && PopStarActive) || (HasScorchStar && (ScorchStarActive = 1))){
		if !fieldPatternShift
			MouseMove windowX+350, windowY+GetYOffset()+100
		if(!DisableToolUse)
			Click "Down"

		if (HasScorchStar && (ScorchStarActive = 1)) {
			nm_setStatus("Test", "active scorch!!!!!!!!!")
		}
...
nm_backgroundEvent(wParam, lParam, *){
	Critical
	global youDied, NightLastDetected, VBState, BackpackPercent, BackpackPercentFiltered, FieldGuidDetected, HasPopStar, PopStarActive, HasScorchStar, ScorchStarActive
	static arr:=["youDied", "NightLastDetected", "VBState", "BackpackPercent", "BackpackPercentFiltered", "FieldGuidDetected", "HasPopStar", "PopStarActive", "HasScorchStar", "ScorchStarActive"]
