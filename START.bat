...
if exist "submacros\moon_macro.ahk" (
	if exist "submacros\AutoHotkey32.exe" (
		if not [%~3]==[] (
			set /a "delay=%~3" 2>nul
			echo Starting Moon Macro in !delay! seconds.
			<nul set /p =Press any key to skip . . . 
			timeout /t !delay! >nul
		)
		start "" "%~dp0submacros\AutoHotkey32.exe" "%~dp0submacros\moon_macro.ahk" %*
		exit
...
if "%exe_missing%" == "1" (
	...
	echo %red% 1. Disable any third-party antivirus software ^(or add the Moon Macro folder as an exception^)%reset%
	...
	echo:
	echo %red%Note: Both Moon Macro and AutoHotkey are safe and work fine with Microsoft Defender^^!%reset%
	echo %red%Join our Discord server for support: discord.gg/r55F5TF82Z%reset%
	echo:
...
if not [!grandparent!] == [] (
	...
	if not [!zip!] == [] (
		...
		if not [!zip!] == [] (
			...
			cd %USERPROFILE%
			for %%a in ("Downloads","Downloads\Moon Macro","Desktop","Documents","OneDrive\Downloads","OneDrive\Downloads\Moon Macro","OneDrive\Desktop","OneDrive\Documents") do (
				if exist "%%~a\!zip!" (
					...
					echo %green%Unzip complete^^! Starting Moon Macro in 10 seconds.%reset%
					<nul set /p =%green%Press any key to skip . . . %reset%
					timeout /t 10 >nul
					start "" "%USERPROFILE%\%%~a\!folder!\submacros\AutoHotkey32.exe" "%USERPROFILE%\%%~a\!folder!\submacros\moon_macro.ahk"
					exit
				)
			)
...
echo %red%Unable to automatically extract Moon Macro^^!%reset%
